...
def register_phillies_alert_test_routes(app):
    ...
    @app.post("/api/sports/generated-alerts/random-test")
    def random_score_alert_test():
        try:
            return no_cache(jsonify(phillies_alert_control.trigger_random_score(core.DB_PATH)))
        except RuntimeError as exc:
            return no_cache(jsonify(error=str(exc))), 409
        except Exception as exc:

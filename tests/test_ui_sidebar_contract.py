...
class ModernUiContractTests(unittest.TestCase):
    ...
    def test_commercial_preview_uses_browser_playback_not_debug_routes(self):
        ...
        self.assertIn("ui_commercial_test.js?v=commercial-test-17", template)
        self.assertIn('id="uiClassifiedCommercialCount"', sidebar)
        self.assertIn('id="uiCommercialDecisionBadge"', sidebar)
        self.assertIn('decisionBadge.classList.toggle("d-none", !Boolean(detector.commercial))', script)
        self.assertIn('id="uiCommercialTestFallback" role="status"', sidebar)
        self.assertNotIn("commercial-in-progress-preview.gif", sidebar)
        self.assertIn("await releaseCommercialPreview();", script)
        self.assertIn("requestPreviewSwitch", script)
        self.assertIn("state.signature_library?.classified", script)
        self.assertIn("releaseCommercialPreview", script)
        self.assertIn("/guide/play/preview/", script)
        self.assertIn('method: "DELETE"', script)
        self.assertIn("ui_sidebar.js?v=sidebar-13", template)

    def test_overview_has_unattended_channel_learning_rotation(self):
        sidebar = (ROOT / "static/js/ui_sidebar.js").read_text(encoding="utf-8")
        script = (ROOT / "static/js/ui_channel_learning_rotation.js").read_text(encoding="utf-8")
        template = (ROOT / "templates/index.html").read_text(encoding="utf-8")
        routes = (ROOT / "api/routes.py").read_text(encoding="utf-8")

        self.assertIn("Unattended Channel Learning", sidebar)
        self.assertIn('id="uiChannelRotationToggle"', sidebar)
        self.assertIn('id="uiChannelRotationMinutes"', sidebar)
        self.assertIn("Probable commercials", sidebar)
        self.assertIn("Channels with data", sidebar)
        self.assertIn("/api/channel-learning-rotation", script)
        self.assertIn("ui_channel_learning_rotation.js?v=channel-rotation-3", template)
        commercial_ui = (ROOT / "static/js/ui_commercial_test.js").read_text(encoding="utf-8")
        self.assertIn("rotationState.running", commercial_ui)
        self.assertIn("collectorPreviewUrl", commercial_ui)
        self.assertIn("register_channel_learning_rotation_routes(app)", routes)

...
class MediaPipelineContractTests(unittest.TestCase):
    def test_settings_and_wizard_expose_encoding_controls(self):
        ...
        self.assertIn("uiCommercialDetectionLock", sidebar)
        self.assertIn("explicitCpu", settings)
        self.assertIn("Locked until FFmpeg encoding is enabled", settings)
        self.assertIn("Select CPU (libx264) explicitly", settings)
